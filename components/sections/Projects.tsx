    ...
    <section id="projects" className="max-w-6xl mx-auto px-6">
      ...
      <SectionReveal>
        <div className="flex items-center gap-3 mb-6">
          <MessageSquare size={20} className="text-[#C2A46B]" />
          <h3 className="text-xl font-bold text-[#F1F5F9]">Insights LinkedIn</h3>
          <a
            href={links.linkedin}
            target="_blank"
            rel="noopener noreferrer"
            className="ml-auto text-sm text-[#C2A46B] hover:text-[#C2A46B] flex items-center gap-1 transition-colors"
          >
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {post.tags.map((tag) => (
                    <span key={tag} className="px-2 py-0.5 rounded text-xs"
                      style={{ background: "rgba(194,164,107,0.1)", color: "#C2A46B" }}>
                      {tag}
                    </span>
                  ))}
                </div>
                <h4 className="text-[#F1F5F9] font-semibold mb-3 group-hover:text-[#C2A46B] transition-colors leading-snug text-sm">
                  {post.title}
                </h4>
                <p className="text-[#94A3B8] text-xs leading-relaxed flex-1 mb-4">{post.excerpt}</p>
                <div className="flex items-center gap-1.5 text-xs text-[#94A3B8]">
                  <Heart size={12} className="text-[#C2A46B]" />
      ...
      <SectionReveal>
        <div className="flex items-center gap-3 mb-6">
          <Icon name="github" size={20} className="text-[#C2A46B]" />
            ...
            <GlowCard className="h-full">
              <a href={p.url} target="_blank" rel="noopener noreferrer" className="block group">
                <div className="flex items-start justify-between mb-3">
                  <span className="font-mono text-[#C2A46B] font-semibold group-hover:underline">
                    {p.title}
                  </span>
      ...
      <SectionReveal>
        <div className="flex items-center gap-3 mb-6">
          <Mic size={20} className="text-[#C2A46B]" />
        ...
        {talks.map((t, i) => (
          <SectionReveal key={t.title} delay={i * 0.07}>
            <div className="glass rounded-xl p-4 flex items-start gap-4"
              ...
              <Icon name={t.icon} size={22} className="text-[#C2A46B] flex-shrink-0 mt-0.5" />
              <div>
                <div className="text-sm font-medium text-[#F1F5F9] mb-1">{t.title}</div>
                <div className="text-xs text-[#C2A46B] flex items-center gap-1">
                  <Eye size={12} /> {t.views}
                </div>
              </div>
            </div>
      ...
      <SectionReveal>
        <div className="flex items-center gap-3 mb-6">
          <BookOpen size={20} className="text-[#C2A46B]" />
        ...
        {blogPosts.map((post, i) => (
          <SectionReveal key={post.title} delay={i * 0.08}>
            <a href={post.url} target="_blank" rel="noopener noreferrer">
              <GlowCard className="group hover:border-[rgba(194,164,107,0.4)] h-full">
                <div className="text-xs text-[#C2A46B] font-mono mb-2">{post.date}</div>
                <h4 className="text-[#F1F5F9] font-semibold mb-4 group-hover:text-[#C2A46B] transition-colors leading-snug">
                  {post.title}
                </h4>
                <div className="flex flex-wrap gap-2">
                  {post.tags.map((tag) => (
