          ...
          <SectionReveal key={post.title} delay={i * 0.08}>
            <a href={links.linkedin} target="_blank" rel="noopener noreferrer">
              <GlowCard className="group h-full flex flex-col">
                ...
                <h4 className="text-[#F1F5F9] font-semibold mb-3 group-hover:text-[#D4AF37] transition-colors leading-snug text-sm">
                  {post.title}
                </h4>
            ...
            <GlowCard className="h-full">
              <a href={p.url} target="_blank" rel="noopener noreferrer" className="block group">
                <div className="flex items-start justify-between mb-3">
                  <span className="font-mono text-[#D4AF37] font-semibold group-hover:underline">
                    {p.title}
                  </span>
        ...
        {talks.map((t, i) => (
          <SectionReveal key={t.title} delay={i * 0.07}>
            <div className="glass rounded-xl p-4 flex items-start gap-4"
              ...
              <Icon name={t.icon} size={22} className="text-[#D4AF37] flex-shrink-0 mt-0.5" />
              <div>
        ...
        {blogPosts.map((post, i) => (
          <SectionReveal key={post.title} delay={i * 0.08}>
            <a href={post.url} target="_blank" rel="noopener noreferrer">
              <GlowCard className="group hover:border-[rgba(212,175,55,0.4)] h-full">
                <div className="text-xs text-[#D4AF37] font-mono mb-2">{post.date}</div>
                <h4 className="text-[#F1F5F9] font-semibold mb-4 group-hover:text-[#D4AF37] transition-colors leading-snug">
                  {post.title}
                </h4>
                <div className="flex flex-wrap gap-2">
                  {post.tags.map((tag) => (
